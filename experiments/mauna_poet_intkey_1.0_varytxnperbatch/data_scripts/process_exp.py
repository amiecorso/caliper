...
parser.add_argument('--tperb', help="txnsperbatch")
        ...
        for interval in intervals:
            ...
            try:
                if not pheader:
                    with open(performance_files[0], 'r') as f:
                        ...
                        perf_out.write("Txns per Batch,TargetInterval(s),Network Size,Run Index," + header + ",Stale Block Rate," + "Num Blocks," + "Num Txns," + "Round Duration," + "My Throughput (tps)," +  "Avg Interval," + "Min Interval," + "Max Interval," + "PercentDiff on TPS\n")
                pheader = True
            ...
            for run in range(REPEATS):
                ...
                if HAVE_ANALYSIS:
                    ...
                    outputline = ",".join([args.tperb, interval, outputline, str(numblocks), str(numtxns), str(duration), str(throughput), str(avg_interval), str(min_interval), str(max_interval), str(percent_diff)])

                else: # DON'T have analysis file
                    outputline = ",".join([args.tperb, interval, outputline, "n/a", "n/a", "n/a", "n/a", "n/a", "n/a/", "n/a", "n/a"])
